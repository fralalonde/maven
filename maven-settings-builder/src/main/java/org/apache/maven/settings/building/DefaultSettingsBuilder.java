...
public class DefaultSettingsBuilder
    ...
    private Settings readSettings(Source settingsSource, SettingsBuildingRequest request,
        ...
        try {
            ...
            problems.add(Problem.Severity.FATAL, "Non-parseable settings " + settingsSource.getLocation()
                    + ": " + e.getMessage(), e.getLineNumber(), e.getColumnNumber(), e);
            return new Settings();
        } catch (IOException e) {
            problems.add(Problem.Severity.FATAL, "Non-readable settings " + settingsSource.getLocation()
                    + ": " + e.getMessage(), -1, -1, e);
            return new Settings();
        }
    ...
    private Settings interpolate(Settings settings, SettingsBuildingRequest request,
        ...
        try {
            ...
            problems.add(Problem.Severity.ERROR, "Failed to interpolate settings: " + e.getMessage(), -1, -1,
                    e);
